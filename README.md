...
真实磁盘的 `/dev/disk*` 节点默认归 `root:operator`，普通用户无法直接读写。本工具的处理方式：

- **默认以读写方式打开块设备**：可执行全部浏览与编辑功能（写操作要求该 ext 卷未被系统挂载，
  系统已挂载的卷会引导先卸载）。
- **打开块设备遇到权限错误时**，界面会弹出确认框，点击“授权并打开”后通过
  `osascript … with administrator privileges` 弹出系统密码框，对目标设备执行
  `chmod a+rw`；设备重新插拔后权限重置，需重新授权。授权横幅合并了所有被拒设备：
  一次密码即可全部授权。
- **只读需求**：启动页勾选“只读打开”即可完全只读访问。
